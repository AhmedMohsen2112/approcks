...
class Ad extends MyModel {
    ...
    private static $columns = ['ads.id', 'ads.lat', 'ads.lng', 'ads.category_id', 'ads.title', 'ads.rate', 'ads.special', 'ads.created_at', 'ads.price', 'ads.mobile', 'ads.email', 
        'categories.form_type', 'categories.parent_id as category_parent', 'categories_translations.title as category', 'users.name', 'locations.id as city_id', 
        'locations.parent_id as country_id', 'locations_translations.title as city', 'ads.details', 'ads.images', 'ads.user_id', 'ads.active', 'currency_translations.title as currency'];
    private static $columns_front = ['ads.id', 'categories.parents_ids as category_parents_ids', 'categories.id as category_id', 'ads.lat', 'ads.lng', 
        'ads.title', 'ads.rate', 'ads.special', 'ads.created_at', 'ads.price', 'ads.mobile',
        'ads.email', 'categories.form_type', 'categories.parent_id as category_parent', 'categories_translations.title as category', 'users.name', 
        'locations.id as city_id', 'locations.parent_id as country_id','users.id as adUserId',
        'locations_translations.title as city', 'ads.details', 'ads.images', 'ads.user_id', 'ads.active', 'currency_translations.title as currency'];

    public function raters() {
        return $this->hasMany(Rating::class, 'entity_id', 'id')
                        ->join('rating_users', 'rating.id', '=', 'rating_users.rating_id')
                        ->join('users', 'users.id', '=', 'rating_users.user_id')
                        ->orderBy('rating_users.created_at', 'desc')
                        ->select(['rating_users.id', 'users.name', 'users.image', 'rating.score', 'rating_users.comment', 'rating_users.created_at']);
    }

    public static function getAdsApi($request, $user, $id = null, $type = null) {
        $lang_code = static::getLangCode();
        $ads = static::join(DB::raw("(SELECT max(id) as id,ad_id
                                        FROM ads_categories
                                        GROUP BY ad_id
                                        ) as ads_cats_1"), function($join) {

                    $join->on("ads.id", "=", "ads_cats_1.ad_id");
                });
        $ads->join(DB::raw("(SELECT id,category_id,ad_id
                                        FROM ads_categories
                                        ) as ads_cats_2"), function($join) {

            $join->on("ads_cats_1.id", "=", "ads_cats_2.id");
        });

         $ads->join('ads_categories as ads_cats_3', 'ads.id', '=', 'ads_cats_3.ad_id');
        $ads->join('categories', 'categories.id', '=', 'ads_cats_2.category_id')
        ...
        $ads = static::handleFromTypeJoin($ads, $request, $user);
        $ads = static::handleFromTypeWhere($ads, $request, $type);
    ...
    public static function getAdsFront($where_array = array()) {
        $lang_code = static::getLangCode();
        ...
        $ads->where('locations_translations.locale', $lang_code);
        $ads->where('categories_translations.locale', $lang_code);
        $ads = static::handleFromTypeJoinFront($ads, $where_array);
        $ads = static::handleFromTypeWhereFront($ads, $where_array);
        $ads->select(static::$columns_front);



        if (isset($where_array['id'])) {
            //dd($where_array);
            $ads->where('ads.id', $where_array['id']);
            $ads = $ads->first();
            if ($ads) {
                $ads = static::transformDetailsFront($ads);
            }
        } else {
            $limit = isset($where_array['limit']) ? $where_array['limit'] : static::$limit;
            $ads->orderBy('ads.created_at', 'desc');
            $ads->groupBy('ads.id');
            if (isset($where_array['limit'])) {
                if (isset($where_array['offset'])) {
                    $ads->offset($where_array['offset']);
                }
                $ads->limit($where_array['limit']);
                $ads = $ads->get();
                $ads = static::transformCollection($ads, 'PaginationFront');
            } else {
                $ads = $ads->paginate(12);

                $ads->getCollection()->transform(function($ad, $key) {
                    return static::transformPaginationFront($ad);
                });
            }
        }
        return $ads;
    ...
    public static function getAdsFront2($where_array = array()) {
        $lang_code = static::getLangCode();
        ...
        //$ads = Ad::join('ads_categories', 'ads.id', '=', 'ads_categories.ad_id')
        $ads->join('categories', 'categories.id', '=', 'ads_cats_2.category_id')
                ->join('categories_translations', 'categories.id', '=', 'categories_translations.category_id')
                ->join('locations', 'ads.city_id', '=', 'locations.id')
                ->join('locations_translations', 'locations.id', '=', 'locations_translations.location_id')
        ...
        if (isset($where_array['id'])) {
            ...
            $ads = $ads->paginate(static::$limit);
            $ads = Ad::transformCollection($ads, 'PaginationFront');
        }
        return $ads;
    }
    ...
    public static function transformPaginationApi($item, $extra_params = array()) {
        ...
        $transformer->title = $item->title ? $item->title : '';
    ...
    public static function transformPaginationFront($item, $extra_params = array()) {
        $lang = static::getLangCode();
        ...
        if (count($ad_images) > 0) {
            ...
            $transformer->image = url('public/uploads/ads') . '/default.png';
        }
        $transformer->is_favourite = $item->is_favourite ? true : false;
        $transformer->favourite_id = $item->is_favourite;
        $transformer->url = _url('ads/ID-' . $item->id);
        return $transformer;
    }
    ...
    public static function transformDetailsFront($item, $extra_params = array()) {
        ...
        $transformer->id = $item->id;
        $transformer->city = $item->city;
        $transformer->category_parents_ids = $item->category_parents_ids;
        $transformer->category_id = $item->category_id;
        $transformer->city_id = (int) $item->city_id;
        $transformer->country_id = (int) $item->country_id;
        $transformer->currency = $item->currency;
        ...
        $transformer->form_type = $item->form_type;
        $transformer->distance = round($item->distance, 1);
        $raters = $item->raters()->paginate(static::$limit);
        $transformer->raters = array_reverse(Rating::transformCollection($raters), true);
        $transformer->total_raters = $raters->total();
        $transformer->images = array();

        $images = $item->images ? json_decode($item->images) : [];
        if (count($images) > 0) {
            foreach ($images as $key => $value) {
                $transformer->images[] = static::rmv_prefix($value);
            }
            $transformer->images = preg_filter('/^/', url('public/uploads/ads') . '/m_', $transformer->images);
            //dd($transformer->images);
        }
        $transformer->name = $item->name;
        $transformer->mobile = $item->mobile;
        $transformer->email = $item->email;
        ...
        if ($item->form_type == 1) {
            ...
            $transformer->rooms_number = (int) $item->rooms_number;
            $transformer->baths_number = (int) $item->baths_number;
            $transformer->furnished = (int) $item->is_furnished;
            $transformer->has_parking = (int) $item->has_parking;
            $transformer->property_type = (int) $item->property_type_id;
            $data = array(
                [
                ...
                [
                    'name' => _lang('app.furnished'),
                    'value' => $item->is_furnished == 1 ? _lang('app.yes') : _lang('app.no')
                ],
                [
                    'name' => _lang('app.parking_place'),
                    'value' => $item->has_parking == 1 ? _lang('app.exist') : _lang('app.none')
                ],
                [
                    'name' => _lang('app.area'),
                    'value' => $item->area . ' ' . _lang('app.m²')
    ...
    public function cats() {
        return $this->hasMany(AdCategory::class, 'ad_id');
    }
    ...
    private static function handleFromTypeJoin($ads, $request, $user) {
        ...
        if ($request->input('lat') && $request->input('lng')) {
            static::$columns[] = DB::raw(static::iniDiffLocations("ads", $request->input('lat'), $request->input('lng')));
        }
        if ($user) {
    ...
    private static function handleFromTypeWhere($ads, $request, $type) {
        ...
        if (isset($options->category) && empty($options->category)) {
            //dd('heres');
            $ads->where('ads_cats_3.category_id', $request->input('category_id'));
        } else if (isset($options->category) && !empty($options->category)) {
            $ads->whereIn('ads_cats_3.category_id', $options->category);
        }
    ...
    private static function handleFromTypeJoinFront($ads, $where_array) {
        $lang_code = static::getLangCode();
        if (isset($where_array['form_type'])) {
            if ($where_array['form_type'] == 1) {
                $ads->join('real_states_ads', 'real_states_ads.ad_id', '=', 'ads.id');
                if (isset($where_array['id'])) {
                    $ads->join('basic_data', 'real_states_ads.property_type_id', '=', 'basic_data.id');
                    $ads->join('basic_data_translations as trans', 'basic_data.id', '=', 'trans.basic_data_id');
                    $ads->where('trans.locale', $lang_code);
                    static::$columns_front = array_merge(static::$columns_front, [
                        "trans.title as property_type", "real_states_ads.has_parking", "real_states_ads.area", "real_states_ads.property_type_id", "real_states_ads.rooms_number",
                        "real_states_ads.baths_number", "real_states_ads.is_furnished"]);
                }
            } else if ($where_array['form_type'] == 2) {
                $ads->join('lands_ads', 'lands_ads.ad_id', '=', 'ads.id');
                static::$columns_front = array_merge(static::$columns_front, ["lands_ads.area"]);
            } else if ($where_array['form_type'] == 3) {
                $ads->join('vehicles_ads', 'vehicles_ads.ad_id', '=', 'ads.id');
                if (isset($where_array['id'])) {
                    ...
                    $ads->where('fuel_type_trans.locale', $lang_code);
                    $ads->where('mileage_trans.locale', $lang_code);
                    static::$columns_front = array_merge(static::$columns_front, ["b1.id as motion_vector_id", "motion_vector_trans.title as motion_vector", "b2.id as engine_capacity_id", "engine_capacity_trans.title as engine_capacity",
                        "b3.id as propulsion_system_id", "propulsion_system_trans.title as propulsion_system", "b4.id as fuel_type_id", "fuel_type_trans.title as fuel_type",
                        "b5.id as mileage_id", "mileage_trans.title as mileage", "vehicles_ads.mileage_unit", "vehicles_ads.status", "vehicles_ads.manufacturing_year"]);
                }
            }
        ...
        if (isset($where_array['lat']) && isset($where_array['lng'])) {
            static::$columns_front[] = DB::raw(static::iniDiffLocations("ads", $where_array['lat'], $where_array['lng']));
        }
        if (isset($where_array['user']) && $where_array['user']) {

            $ads->leftJoin('favourites', function($join) use($where_array) {
                $join->on('favourites.ad_id', '=', 'ads.id')
                        ->where('favourites.user_id', $where_array['user']->id);
            });
            static::$columns_front[] = "favourites.id as is_favourite";
        }

        return $ads;
    }

    private static function handleFromTypeWhereFront($ads, $where_array) {
        if (isset($where_array['search'])) {
            $ads->whereRaw(handleKeywordWhere(['ads.title', 'ads.details'], $where_array['search']));
        }
        if (isset($where_array['country_id'])) {
            $ads->where('ads.country_id', $where_array['country_id']);
        }
        if (isset($where_array['city_id'])) {
            $ads->where('ads.city_id', $where_array['city_id']);
        }
        if (isset($where_array['category_id'])) {
            $ads->where('ads_cats_3.category_id', $where_array['category_id']);
        }
        if (isset($where_array['user_id'])) {
            $ads->where('ads.user_id', $where_array['user_id']);
        }
        if (isset($where_array['special'])) {
            $ads->where('ads.special', $where_array['special']);
        }
        if (isset($where_array['option'])) {
            if ($where_array['option'] == 'favorites') {
                $ads->whereNotNull('favourites.id');
            }
            if ($where_array['option'] == 'for_auth_user') {
                $ads->where('ads.user_id', $where_array['user']->id);
            }
        }

        if (isset($where_array['filter'])) {
            $filter = $where_array['filter'];
            if (isset($filter['price_from'])) {
                $ads->where('ads.price', '>=', $filter['price_from']);
            }
            if (isset($filter['price_to'])) {
                $ads->where('ads.price', '<=', $filter['price_to']);
            }
            ...
            if (isset($filter['show'])) {
                $filter['show'] = explode('-', $filter['show']);
                if (in_array(1, $filter['show'])) {
                    $ads->where('ads.created_at', '>=', Carbon::today());
                }
                if (in_array(2, $filter['show'])) {
                    $ads->whereNotNull('ads.title');
                }
                if (in_array(3, $filter['show'])) {

                    $ads->whereNotNull('ads.images');
                }
//            if (in_array(4, $where_array['show'])) {
//                $ads->orderBy('distance');
//            }
            }
            ...
            if ($where_array['form_type'] == 1) {
                if (isset($filter['property_type'])) {
                    $ads->whereIn('real_states_ads.property_type_id', explode('-', $filter['property_type']));
                }
                if (isset($filter['rooms_number_from'])) {
                    $ads->where('real_states_ads.rooms_number', '>=', $filter['rooms_number_from']);
                }
                if (isset($filter['rooms_number_to'])) {
                    $ads->where('real_states_ads.rooms_number_to', '<=', $filter['rooms_number_to']);
                }
                if (isset($filter['baths_number_from'])) {
                    $ads->where('real_states_ads.baths_number_from', '>=', $filter['baths_number_from']);
                }
                if (isset($filter['baths_number_to'])) {
                    $ads->where('real_states_ads.baths_number_to', '<=', $filter['baths_number_to']);
                }
                if (isset($filter['furnished'])) {
                    $ads->whereIn('real_states_ads.is_furnished', explode('-', $filter['furnished']));
                }
                if (isset($filter['has_parking'])) {
                    $ads->whereIn('real_states_ads.has_parking', explode('-', $filter['has_parking']));
                }
                if (isset($filter['area_from'])) {
                    $ads->where('real_states_ads.area', '>=', $filter['area_from']);
                }
                if (isset($filter['area_to'])) {
                    $ads->where('real_states_ads.area', '<=', $filter['area_to']);
                }
            }
            if ($where_array['form_type'] == 2) {

                //lands
                if (isset($filter['area_from'])) {
                    $ads->where('lands_ads.area', '>=', $filter['area_from']);
                }
                if (isset($filter['area_to'])) {
                    $ads->where('lands_ads.area', '<=', $filter['area_to']);
                }
            }
            if ($where_array['form_type'] == 3) {
                //cars
                if (isset($filter['status'])) {
                    //dd(explode('-', $filter['status']));
                    $ads->whereIn('vehicles_ads.status', explode('-', $filter['status']));
                }
                if (isset($filter['manufacturing_year_from'])) {
                    $ads->where('vehicles_ads.baths_number_from', '>=', $filter['manufacturing_year_from']);
                }
                if (isset($filter['manufacturing_year_to'])) {
                    $ads->where('vehicles_ads.manufacturing_year', '<=', $filter['manufacturing_year_to']);
                }
                if (isset($filter['motion_vector'])) {
                    $ads->whereIn('vehicles_ads.motion_vector_id', explode('-', $filter['motion_vector']));
                }
                if (isset($filter['engine_capacity'])) {
                    $ads->whereIn('vehicles_ads.engine_capacity_id', explode('-', $filter['engine_capacity']));
                }
                if (isset($filter['propulsion_system'])) {
                    $ads->whereIn('vehicles_ads.propulsion_system_id', explode('-', $filter['propulsion_system']));
                }
                if (isset($filter['fuel_type'])) {
                    $ads->whereIn('vehicles_ads.fuel_type_id', explode('-', $filter['fuel_type']));
                }
            }
        }
        return $ads;
